...
# sys.path: 确保 robots 包可导入(omni_29dof_v260705 在项目根目录)
_this = Path(__file__).resolve()
_project_root = _this.parent.parent.parent.parent.parent
...
from .mdp.jump_commands import JumpMotionCommand
from .mdp.obs529 import obs529
# V11: V7 终止函数(bad_anchor_ori/fell/out_of_bounds, jump_high vendored mdp 没有)
from .mdp.v7_terminations import bad_anchor_ori, fell, out_of_bounds
# V11: V7 奖励栈(26 项, 见 v7_rewards.py)。jump_high 原自定义奖励(jump_phase_rewards/
# jump_rewards)已全部移除, 换成 V7 算法。
from .mdp.jump_rewards import (
    arm_back_penalty,
    arm_tracking_exp,
    boundary_penalty,
    elbow_bend_penalty,
...
_DEFAULT_MOTION_FILE = str(Path(__file__).resolve().parent.parent.parent.parent.parent / "training_data" / "jump_high_firstjump_50fps.npz")
...
class OmniJumpEnvCfg(TrackingEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.rewards.jump_height_bonus = RewTerm(
            ...
            # 2026-08-16: 回调 10→8 后恢复 8→9(用户强调目标 1.05 不变, 不能只保稳定;
            # 小幅降体现稳定考量, 不放弃高度)
            weight=9.0,
        ...
        self.rewards.takeoff_vertical_vel = RewTerm(
            ...
            # 2026-08-16: 回调 10→8 后恢复 8→9(目标不变, 稳定靠回稳机制而非降高)
            weight=9.0,
